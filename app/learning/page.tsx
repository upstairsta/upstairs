...
// 🌟 HELPER: Automatically assigns correct categories based on track codes
// This prevents database loads from wiping out your categories!
const assignCategoryToCourse = (course: Course): Course => {
  const socialTracks = ["GD", "Video", "Photo", "SM", "W", "CC", "I", "UDC", "T", "Y", "Tech", "LS", "Health"];
  const opsTracks = ["Admin", "HR", "Finance", "PM", "CS", "VA"];
  const techTracks = ["DEV", "DB", "IT", "AI", "Cyber", "DevOps", "UI/UX"];
  const bizTracks = ["TK", "DM", "AM", "BM"];
  const eduTracks = ["Trainers"];

  let category = "Operations"; // Default fallback

  if (socialTracks.includes(course.track)) {
    category = "Content, Social Media & Influence";
  } else if (opsTracks.includes(course.track)) {
    category = "Operations";
  } else if (techTracks.includes(course.track)) {
    category = "Tech & Infrastructure";
  } else if (bizTracks.includes(course.track)) {
    category = "Business & Marketing";
  } else if (eduTracks.includes(course.track)) {
...
export default function LearningPortalPage() {
  // Initialize local courses with guaranteed categories
  const [courses, setCourses] = useState<Course[]>(() => 
    initialCourses.map(assignCategoryToCourse)
  );
  ...
  // Track active Category Filtering
  const [selectedCategory, setSelectedCategory] = useState('All'); 

  const [selectedCourse, setSelectedCourse] = useState<Course | null>(null);
  const [activeLesson, setActiveLesson] = useState<Lesson | null>(null);

  // Extract list of unique categories to draw our buttons
  ...
  const loadPortalData = async () => {
    try {
      ...
      if (coursesRes.data) {
        ...
        initialCourses.forEach(c => combinedMap.set(c.id, assignCategoryToCourse(c)));
        
        // Merge in backend courses, making sure to apply/preserve our grouping categories
        backendCourses.forEach(c => {
          combinedMap.set(c.id, assignCategoryToCourse(c));
        });
  ...
  // 🔍 Filtered logic combining active search terms with our selected category group!
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
                      ...
                      <div className="xl:col-span-8 space-y-5">
                        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 pb-2.5">
                          ...
                          {/* 🛠️ CATEGORY GROUP FILTER BUTTONS IN ACTION */}
